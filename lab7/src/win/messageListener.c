...
int main(int argc, char* argv[]) {
    ...
    HANDLE messagePipe;
    if (isFirst) {
        messagePipe = CreateFile("COM1", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
        messagePipe = CreateFile("COM1", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
     ...
     //        );
        ...
        messagePipe = CreateFile("COM2", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
        messagePipe = CreateFile("COM2", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
     ...
     if (messagePipe == INVALID_HANDLE_VALUE) {
        printf("Cannot open pipe at messageListener, error: %ld, exit 2\n", GetLastError());
        exit(2);
    }
    while(1) {
    	if (isFirst) {
